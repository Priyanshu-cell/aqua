...
const DomesticRoTen = ({ productDataTen }) => {
  if (!productDataTen) {
    return (
    ...
    <div className="p-6 md:px-16 lg:px-32 mx-auto bg-gradient-to-r from-indigo-100 via-purple-100 to-pink-100 shadow-xl space-y-6 w-full product-background-related">
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 gap-8">
        ...
        <div className="flex justify-center items-center">
          <img
            src={productDataTen.image}
            alt={productDataTen.name}
            className="w-full h-96 object-contain rounded-lg shadow-2xl transform transition duration-500 hover:scale-105 lg:size-[34rem]"
          />
        </div>
        ...
        <div className="bg-yellow-900 p-6 rounded-lg shadow-md">
          ...
          <ul className="space-y-4 text-white">
            {Object.entries(productDataTen.specifications).map(([key, value]) => (
              <li key={key}>
                ...
                {Array.isArray(value) ? (
                  ...
                  <ul className="list-disc list-inside ml-4 space-y-2">
                    {Object.entries(value).map(([subKey, subValue]) => (
                      <li key={subKey}>
                        <strong className="text-indigo-300">{subKey.replace(/([A-Z])/g, " $1")}: </strong>
                        {subValue}
                      </li>
                    ))}
                  </ul>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        ...
        <ul className="space-y-4">
          {productDataTen.benefits?.map((benefit, index) => (
