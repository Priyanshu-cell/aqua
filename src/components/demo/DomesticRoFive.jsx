const DomesticRoFive = ({ productDataFive }) => {
  ...
  return (
    <div className="p-6 md:px-16 lg:px-32 mx-auto bg-gradient-to-r from-blue-100 via-teal-100 to-green-100 shadow-xl space-y-6 w-full product-background-related">
      {/* Product Name */}
      <h2 className="text-4xl md:text-5xl font-extrabold text-center text-white hover:text-blue-600 transition duration-300 ease-in-out">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-yellow-900 p-6 rounded-lg shadow-md">
          <h3 className="text-3xl font-semibold mb-6 text-white sub-heading">Specifications:</h3>
          <ul className="space-y-4">
            {Object.entries(productDataFive.specifications).map(([key, value], index) => (
              <li key={index} className="text-lg text-gray-200">
                <strong className="text-xl text-indigo-400 capitalize">{key.replace(/([A-Z])/g, " $1").trim()}:</strong>
                {Array.isArray(value) ? (
      ...
      <div className="bg-gray-100 p-6 rounded-lg shadow-md">
        <h3 className="text-3xl font-semibold mb-6 text-blue-600">Description:</h3>
